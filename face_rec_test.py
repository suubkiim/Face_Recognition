...
# face_name = ['seojin','Subin_Kim']
face_name = ['Emma_stone', 'Hyeongwon_Kang', 'Jeongseob_Kim', 'Seojin_Lee', 'Subin_Kim']
...
# test
img_num = 1
cap_img = f'test_img/{img_num}.png'
cap = cv2.VideoCapture(cap_img)
...
while True:
    ...
    for i in range(detect.shape[0]):
        ...
        # for test
        crop = frame[y1+10:y2, x1:x2]
#         crop = frame[y1:y2, x1:x2]
        face_idx, confidence = face_recognition(recognition_net, crop)

        # for test
        cv2.rectangle(frame, (x1, y1+30), (x2, y2), (0, 255, 0))
